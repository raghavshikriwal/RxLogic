...
Wires together the three independent reasoning stages behind one call:
    ...
    2. Planner.build_plan()             -> internally runs CSPScheduler,
...
should call. Neither the rule engine, the CSP scheduler, nor the planner
should be invoked directly outside this service, so there is exactly one
place where the reasoning pipeline order is defined (Section 4.1).
"""
...
class ReasoningService:
    """Orchestrates rule engine -> planner (-> CSP) into one call."""
